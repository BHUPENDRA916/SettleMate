  ...
  return (
    <form onSubmit={handleSubmit} autoComplete="off" className="space-y-4">
      ...
      <div className="space-y-6">
        <button disabled={loading} type="submit" className="w-full inline-flex items-center justify-center rounded-xl bg-teal-600 px-5 py-2.5 text-white font-semibold shadow-sm hover:bg-teal-700 active:bg-teal-700/90 transition-all focus:outline-none focus:ring-2 focus:ring-teal-500/60">{loading ? 'Creating...' : 'Create account'}</button>
      </div>
    </form>
  );
}